...
echo "🚀 Testing Humbu Enterprise Platform"
echo "===================================="
...
curl -s -X POST http://localhost:8001/chat \
  ...
  -d '{"message": "Direct AI test"}' | python3 -m json.tool
...
curl -s -X POST http://localhost:8102/api/v1/ai/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "Gateway integration test"}' | python3 -m json.tool

echo ""
echo "6. Testing Platform Info..."
curl -s http://localhost:8102/api/platform | python3 -m json.tool
...
# Cleanup function
cleanup() {
    kill $AI_PID $GATEWAY_PID 2>/dev/null
